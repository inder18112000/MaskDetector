...
class dasboard_light:
    ...
    def addloc(self):
        self.rootadd = Toplevel(self.rootl)
        ...
        Label(f, text="New Surveillance Location",
              font=theme.F_SUB, bg=CARD, fg=TEXT).pack(pady=(0, 14))
    ...
    def sur(self):
        ...
        db_complaints.update_status(str(self.s_loc.get()), "Surviellance done successfully")
    ...
    def stop_camera(self):
        ...
        ax.set_title("Mask Compliance", fontsize=13, fontweight='bold', color=TEXT)
    ...
    def __init__(self, result):
        ...
        self.rootl.iconphoto(False, self.icon_photo)
        self.rootl.config(bg=BG)
        self.rootl.minsize(900, 600)
        ...
        nav = Frame(self.rootl, bg=PANEL, height=50)
        ...
        # ── Bottom bar ────────────────────────────────────────────────────────
        bottom = Frame(self.rootl, bg=CARD, height=60)
        ...
        theme.btn(bottom, "🌙  Dark Mode", self.dark_mode,
                  bg=theme.D_PANEL, width=150).pack(side=LEFT, padx=8, pady=10)

        theme.btn(bottom, "⏻  Logout", self.lg,
                  bg=theme.DANGER, width=130).pack(side=RIGHT, padx=16, pady=10)

        # ── Left sidebar ──────────────────────────────────────────────────────
        self.bf1 = Frame(self.rootl, bg=PANEL, width=300)
        ...
        Label(info_frame, text=f"[ {result[3]} ]",
              font=theme.F_SMALL, bg=PANEL, fg="#b3c8f5").pack()
        Label(info_frame, text=result[1],
              font=theme.F_SMALL, bg=PANEL, fg="#b3c8f5").pack(pady=(2, 0))

        Frame(self.bf1, bg="#4a90d9", height=1).pack(fill=X, padx=16, pady=8)

        # Surveillance form labels
        ...
        self.mode = Entry(cam_frame, font=theme.F_ENTRY, width=6,
                          ...
                          highlightcolor=FG, bg="#2a63c8", fg=FG,
        ...
        entry_cfg = dict(font=theme.F_ENTRY, width=18, relief=FLAT,
                         bg="#2a63c8", fg=FG, disabledbackground="#2a63c8",
                         disabledforeground="#a8c8f8",
                         highlightthickness=1, highlightbackground="#4a90d9")
        ...
        # ── Main content area ─────────────────────────────────────────────────
        self.main_frame = Frame(self.rootl, bg=BG)
